...
const fetchData = async () => {
   ...
   if (data.jobTitle == "") jobTitle.innerHTML = "لا يوجد";
   else jobTitle.innerHTML = data.jobTitle;
...
accept.addEventListener("click", async () => {
   const res = await fetch(
      `http://localhost:3000/api/v1/user/requests/${id}/accept`,
      {
         method: "GET",
         headers: {
            "Content-Type": "application/json",
   ...
   window.location.href = "receivedConnections.html";
});
...
reject.addEventListener("click", async () => {
   const res = await fetch(
      `http://localhost:3000/api/v1/user/requests/${id}/reject`,
      {
         method: "GET",
         headers: {
            "Content-Type": "application/json",
